                                    ...
                                    PyObject* args)
{
...

  {
    ...
    for (iter_state = 0; iter_state < tm.num_states; iter_state++)
    {
      ...
      for (iter_symbol = 0; iter_symbol < tm.num_symbols; iter_symbol++)
      {
        ...
        tm.machine[iter_state].t[iter_symbol].d = i1;
  ...
  tm.tape = (int *)calloc(tm.tape_length * tm.tape_length,sizeof(*(tm.tape)));
  ...
  tm.position_x = tape_middle;

  tm.max_down   = tape_middle;
  tm.max_up     = tape_middle;
  tm.position_y = tape_middle;

  tm.symbol = tm.tape[INDEX(&tm)];
  tm.state  = 0;
