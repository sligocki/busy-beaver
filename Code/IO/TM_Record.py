...
def parse_tm(line: str) -> Turing_Machine.Simple_Machine:
  ...
  return Turing_Machine.tm_from_quintuples(quints, states=list(range(num_states)), symbols=list(range(max_symbol + 1)))


def _pack_trans_ints(symbol: int, dir: int, state: int, is_newrow: bool) -> int:
  if dir not in [0, 1]:
    dir = 0
...
def _pack_trans(trans: Turing_Machine.Transition, is_newrow: bool) -> int:
  if trans.condition == Turing_Machine.UNDEFINED:
    return _pack_trans_ints(symbol=-1, dir=0, state=-1, is_newrow=is_newrow)
  else:
    return _pack_trans_ints(
      symbol=trans.symbol_out,
      dir=trans.dir_out,
...
def _unpack_tm(pack: bytes) -> Turing_Machine.Simple_Machine:
  ...
  for byte in pack:
    ...
    symbol_in += 1
  return Turing_Machine.tm_from_quintuples(quints, states=list(range(state_in + 1)), symbols=list(range(symbol_in)))
...
def tm_from_list(tm_list: io_pb2.TMList) -> Turing_Machine.Simple_Machine:
  ...
  return Turing_Machine.tm_from_quintuples(
    quints,
...
class TM_Record:
  ...
  def tm_enum(self):
    if not self.tme:
      ...
      self.tme = TM_Enum.TM_Enum(tm, allow_no_halt=self.proto.tm.allow_no_halt)
    return self.tme
