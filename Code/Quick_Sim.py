...
def run(machine, block_size, back, prover, recursive, options):
  ...
  if not block_size:
    ...
    Block_Finder.block_finder(machine, options, bf_info.parameters, bf_info.result)
    block_size = bf_info.result.best_block_size

  # Do not create a 1-Block Macro-Machine (just use base machine)
  if block_size != 1:
    machine = Turing_Machine.Block_Macro_Machine(
      machine, block_size, max_sim_steps_per_symbol=options.max_steps_in_block
    )
  if back:
  ...
  try:
    if options.quiet or options.verbose:  # Note verbose prints inside sim.step()
      ...
      while sim.op_state == Turing_Machine.RUNNING and (options.max_loops == 0 or total_loops < options.max_loops):
        sim.step()
        total_loops += 1
    else:
      total_loops = 0
  ...
  if options.print_macro_ttable and isinstance(machine, Turing_Machine.Macro_Machine):
    ...
    table = [["" for _ in range(len(macro_symbols) + 1)] for _ in range(len(macro_states) + 1)]
    ...
    print()
    print(
      "Macro Machine TTable (%d states, %d symbols, %d transitions) [n] is base steps per transition:"
...
if __name__ == "__main__":
  ...
  parser.add_option(
    ...
    "--verbose",
    action="store_true",
    help="Print step-by-step information from simulator and prover (Overrides other --verbose-* flags).",
  )
  ...
  parser.add_option(
    ...
    type=int,
    default=0,
    help="Specify a maximum number of loops (0 for infinite). [Default: %default]",
  )
  parser.add_option(
    "--bf-loops",
    type=int,
    default=10_000,
    help="Number of steps to run Block Finder. [Default: %default]",
  )
  parser.add_option(
    "--freeze-prover",
    action="store_true",
  ...
  parser.add_option("--max-steps-in-block", type=int, default=1_000_000, help="[Default: %default]")
  parser.add_option("--max-steps-in-backsymbol", type=int, default=1_000, help="[Default: %default]")
  ...
  parser.add_option("--latex", action="store_true", help="Print score in LaTeX math format")
