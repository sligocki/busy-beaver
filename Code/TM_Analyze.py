...
class TMStats:
  ...
  def add_record(self, tm_record: IO.TM_Record):
    ...
    self.num_finite_linear_rules_proven.add(proto.filter.simulator.result.num_finite_linear_rules_proven)
  ...
  def print(self):
    ...
    if self.sim_num_loops.count:
      ...
      print(f"  - num_linear_rules      : Mean {self.num_finite_linear_rules_proven.mean():9_.0f}  Max {self.sim_num_finite_linear_rules_proven.max_value:9_d}  (Set in {self.sim_num_finite_linear_rules_proven.count / self.count:7.2%})")
