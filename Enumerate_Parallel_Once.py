...
class Enumerator(object):
  ...
  def continue_enum(self):
    ...
    while len(self.stack) > 0:
      ...
      if cur_tm == 0:
        etime = time.time()
        print "  stopped %d (%d) - %f (%f)..." % (mpi.rank,len(self.stack),etime-stime,self.stack.ttime)
        sys.stdout.flush()
        return
      # ... and run it
...
if __name__ == "__main__":
  ...
  if rank == 0:
    ...
    # Enumerate_Parallel_Once.py may be sent an infile param but it should be ignored
    ...
    print "Enumerate_Parallel_Once.py --steps=%s --time=%s --save_freq=%s --seed=%s --outfile=%s --checkpoint=%s%s --states=%s --symbols=%s" % \
    ...
    checkpoint = checkpoint + ".%05d" % rank
    
    # io = IO(None, outfile, log_number, True)
    io = IO(None, outfile, log_number, False)

    enumerator = Enumerator_Startup(states, symbols, steps, 
