#! /usr/bin/env python
#
# Enumerate_Parallel_MW_Newer.py
#
...
class Stack(list):
  ...
  def recvItem(self):
    if len(self.local) > 0:
      ...
      stime = time.time()
      item,status = mpi.recv(0)
      etime = time.time()
      self.ttime += etime - stime
      # print "  worker %d received: %s (%d)" % (mpi.rank,item,status.tag)
      # sys.stdout.flush()
  ...
  def sendList(self, items):
    ...
    # sys.stdout.flush()
    if len(items) > 0 and len(self.local) == 0:
      self.local = items
      msg = [1,[]]
    else:
      if len(self.local) == 0:
...
if __name__ == "__main__":
  if mpi.rank == 0:
    ...
    # Enumerate_Parallel_MW_Newer.py may be sent an infile param but it should be ignored
    ...
    print "Enumerate_Parallel_MW_Newer.py --steps=%s --time=%s --save_freq=%s --seed=%s --outfile=%s --checkpoint=%s%s --states=%s --symbols=%s" % \
